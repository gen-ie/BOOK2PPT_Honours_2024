...
# add dropout to base model
model = DOC2PPTWithDropout(base_model)
...
scheduler = ReduceLROnPlateau(optzr, mode='min', factor=0.1, patience=3)
...
with tqdm(range(100), ascii=True) as TQ:
    for e in TQ:
        ...
        if ls_vl < best_val_loss:
            best_val_loss = ls_vl
            patience_counter = 0
            T.save(model.state_dict(), '%s/storybook_model_dropout_best.pt'%(PATH))  # Save the best model
        else:
            patience_counter += 1
...
T.save(model.state_dict(), '%s/storybook_model_dropout_final.pt'%(PATH))