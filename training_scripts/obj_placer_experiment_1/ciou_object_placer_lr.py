...
if __name__=='__main__':
    ...
    PATH = 'ciou_orig_locmodel_lr_sched'
    ...
    with tqdm(range(75), ascii=True) as TQ:
        for e in TQ:
            ...
            if ls_vl < best_val_loss:
                best_val_loss = ls_vl
                patience_counter = 0
                T.save(loc_model.state_dict(), '%s/ciou_locmodel_best_lr.pt'%(PATH))  # Save the best model
            else:
                patience_counter += 1
                # if patience_counter >= patience:
                #     print("Early stopping triggered. Training stopped.")
                #     break
    T.save(loc_model.state_dict(), '%s/ciou_locmodel_final_lr.pt'%(PATH))
